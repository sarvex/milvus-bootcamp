...
class MySQLHelper():
    ...
    def create_mysql_table(self, table_name):
        ...
        sql = f"create table if not exists {table_name}(milvus_id TEXT, seq_class TEXT);"
        try:
    ...
    def create_class_table(self, table_name):
       # Create mysql table to store dna sequence labels
        self.test_connection()
        sql = f"create table if not exists {table_name}(seq_class TEXT, gene_family TEXT);"
        try:
            self.cursor.execute(sql)
            LOGGER.debug(f"MYSQL create table: {table_name} with sql: {sql}")
        except Exception as e:
    ...
    def load_data_to_mysql(self, table_name, data):
        ...
        self.test_connection()
        sql = f"insert into {table_name} (milvus_id,seq_class) values (%s,%s);"
        try:
    ...
    def load_data_to_class(self, table_name, data):
        ...
        self.test_connection()
        sql = f"insert into {table_name} (seq_class,gene_family) values (%s,%s);"
        try:
            self.cursor.executemany(sql, data)
            self.conn.commit()
            LOGGER.debug(f"MYSQL loads data to table: {table_name} successfully")
    ...
    def search_by_milvus_ids(self, ids, table_name):
        ...
        sql = f"select * from {table_name} where milvus_id in ({str_ids}) order by field (milvus_id,{str_ids});"
        try:
            self.cursor.execute(sql)
